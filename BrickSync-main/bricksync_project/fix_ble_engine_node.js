...
const sStr = "            if (window.parent.connectionType === 'BLE') {";
const eStr = "            try {\n                await writeToHub('\\x03\\x03');";

let s = html.indexOf(sStr);
let e = html.indexOf(eStr);

if (s === -1 || e === -1) {
    console.log("MARKERS NOT FOUND! Retrying with regex...");
    const regex = /if \(window\.parent\.connectionType === 'BLE'\) \{[\s\S]*?return;\s*\}/;
    const match = html.match(regex);
    if (match) {
        s = match.index;
        e = s + match[0].length;
        console.log("Found with regex!", s, e);
    } else {
...
const newBLE = `if (window.parent.connectionType === 'BLE') {
...
html = html.substring(0, s) + newBLE + html.substring(e);
fs.writeFileSync('web_app/lesson.html', html, 'utf8');
console.log("SUCCESS NODE REGEX");
